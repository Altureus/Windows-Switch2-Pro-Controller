...
| Exact button-bit + stick-byte mapping | ✅ done (`mapping_data.py`, via `map_buttons.py`) |
...
| `mapping_data.py`   | the verified button/stick map (generated by `map_buttons.py`) |
...
| File | Purpose |
|---|---|
| `map_buttons.py`    | guided re-mapper — names each control, writes `mapping_data.py` |
...
4. ✅ Verified button/stick map (`map_buttons.py` → `mapping_data.py`); GL/GR → Z.
